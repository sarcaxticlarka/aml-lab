...
def minibatch_gradient_descent_mlr(X, y, theta, alpha, epochs, batch_size):
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(X), batch_size):
            ...
            predictions = predict_mlr(X_batch, theta)

            residual = compute_mlr_residual(y_batch, predictions)

            gradient = compute_mlr_gradient(X_batch, residual)

            theta = update_mlr_parameters(theta, gradient, alpha)
    
        predictions = predict_mlr(X, theta)
